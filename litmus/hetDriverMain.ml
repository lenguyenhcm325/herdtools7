...
let dump_launch_geometry dialect identity ch =
  ...
  int _noiseBlocks = HET_GPU_NOISE_BLOCKS;
  if (_noiseBlocks < 0) _noiseBlocks = 0;
  int _stressBlocks = (HET_GPU_STRESS_BLOCKS >= 0) ? HET_GPU_STRESS_BLOCKS
...
|} ;
  ...
  s {|  if (HET_GPU_MEM_STRESS_PCT > 0 && _stressBlocks == 0)
    fprintf(stderr, "HetLitmus WARNING: the mem-stress population is EMPTY (test=%d + noise=%d fills the co-resident cap %d).  HET_GPU_MEM_STRESS_PCT=%d asks for scratchpad stress and NO block will do any.\n",
            _testBlocks, _noiseBlocks, _maxGrid, (int)HET_GPU_MEM_STRESS_PCT);
  uint32_t _pre_pat = (uint32_t)HET_GPU_PRE_STRESS_PATTERN;
  uint32_t _mem_pat = (uint32_t)HET_GPU_MEM_STRESS_PATTERN;
...
let dump_gpu_stress_alloc dialect ch =
  ...
  s (Printf.sprintf "  uint32_t *_scratch; %s\n"
       (dialect.gd_dev_malloc "_scratch" "sizeof(uint32_t)*HET_GPU_SCRATCH_WORDS")) ;
...
let dump_cpu_stress_setup procs ch =
  ...
  s {|  int _nCpuStress = HET_CPU_STRESS_THREADS;
  if (_nCpuStress < 0) {
    _nCpuStress = _ncores - _nCpuTest - HET_CPU_NOISE_THREADS - HET_CPU_RESERVE_CORES;
    if (_nCpuStress < 0) _nCpuStress = 0;
  }
  het_cpu_tally _ct;
  int _stress_go = 0;
  ...
  uint32_t _cpu_nregions = (uint32_t)(HET_CPU_SCRATCH_WORDS / HET_CPU_WORDS_PER_REGION);
  if (_cpu_nregions < 1) _cpu_nregions = 1;
  uint32_t _cpu_spread = HET_CPU_SPREAD;
  if (_cpu_spread > _cpu_nregions) {
    fprintf(stderr, "HetLitmus WARNING: realised CPU stress spread is %u region(s), not HET_CPU_SPREAD=%d -- the CPU stress scratchpad (HET_CPU_SCRATCH_WORDS/HET_CPU_WORDS_PER_REGION) holds only %u.  The CPU stress is weaker than the configuration says.\n",
  ...
  het_cpu_stress_args *_sa = (het_cpu_stress_args*)malloc_check(sizeof(het_cpu_stress_args)*(_nCpuStress>0?_nCpuStress:1));
  pthread_t *_sth = (pthread_t*)malloc_check(sizeof(pthread_t)*(_nCpuStress>0?_nCpuStress:1));
|}
...
let dump_noise_setup ch =
  ...
  int _cpu_noise_n = 0;
  uint64_t _noise_slice = HET_CPU_NOISE_THREADS > 0 ? _noise_words / HET_CPU_NOISE_THREADS : 0;
...
let dump_run_spawn_stress ch =
  ...
  s {|    _ct.preload_inert = !het_cpu_preload_live();
    ...
    int _stress_core0 = HET_CPU_FIRST_CORE + _nCpuTest + HET_CPU_NOISE_THREADS;
    if (_aff && _stress_core0 + _nCpuStress > _ncores)
      fprintf(stderr, "HetLitmus WARNING: %d stress thread(s) from core %d exceed %d core(s) -- stress-thread pins WRAP onto the test threads' cores, so the test threads no longer have a core to themselves.\n",
              _nCpuStress, _stress_core0, _ncores);
    for (int _e = 0; _e < _nCpuStress; ++_e) {
      uint32_t _off = (_cpu_nregions > _cpu_spread)
                    ? ((uint32_t)_e * _cpu_spread) % (_cpu_nregions - _cpu_spread + 1)
                    : 0u;
      _sa[_e].scratch          = _cpu_scratch;
      _sa[_e].idx              = _cpu_idx + _off;
      _sa[_e].nidx             = _cpu_spread;
      _sa[_e].words_per_region = (uint32_t)HET_CPU_WORDS_PER_REGION;
      _sa[_e].pattern          = (uint32_t)HET_CPU_STRESS_PATTERN;
      _sa[_e].core             = _aff ? ((_stress_core0 + _e) % _ncores) : -1;
      _sa[_e].go               = &_stress_go;
      _sa[_e].tally            = &_ct;
    ...
    if (_noise != NULL && HET_CPU_NOISE_THREADS > 0) {
      ...
      for (int _t = 0; _t < HET_CPU_NOISE_THREADS; ++_t) {
        ...
        _na[_t].tally           = &_ct;
        if (pthread_create(&_nth[_t], NULL, het_cpu_noise, &_na[_t]) == 0) _cpu_noise_n++;
      }
    }
|}
...
let dump_run_reset_observation dialect memory procs ch =
  ...
  s (Printf.sprintf "    %s\n"
       (dialect.gd_dev_memset0 "_scratch" "sizeof(uint32_t)*HET_GPU_SCRATCH_WORDS")) ;
  s (Printf.sprintf "    %s\n"
...
let dump_run_spawn_cpu_threads procs memory ch =
  ...
  List.iter
    (fun cp ->
      ...
      let core =
        Printf.sprintf
          "(_aff ? ((HET_CPU_FIRST_CORE + %d) %% _ncores) : -1)" !ti in
      incr ti ;
      let fields =
        String.concat ", "
          (List.map snd addr
...
let dump_run_join dialect procs ch =
  ...
  s {|    __atomic_store_n(&_stress_go, 0, __ATOMIC_RELAXED);
    for (int _e = 0; _e < _nCpuStress; ++_e) pthread_join(_sth[_e], NULL);
    for (int _t = 0; _t < _cpu_noise_n; ++_t) pthread_join(_nth[_t], NULL);
|}
...
let dump_run_stress_report dialect ch =
  ...
  s (Printf.sprintf "    %s\n"
       (dialect.gd_memcpy_d2h "_stress_tally_h" "_stress_tally"
          "sizeof(uint32_t)*HET_TALLY_N")) ;
  s {|    {
      unsigned long long _sr = _ct.stress_rounds;
...
let dump_run_record_stamp identity ch =
  ...
  s {|    _rec.stress_requested =
      ...
      | ((HET_CPU_NOISE_THREADS > 0) ? HET_REQ_CPU_NOISE : 0u)
      | ((_noiseBlocks > 0) ? HET_REQ_GPU_NOISE : 0u);
    _rec.stress_truncated = _stress_tally_h[HET_TALLY_TRUNC];
    _rec.gpu_stress_rounds = _stress_tally_h[HET_TALLY_STRESS_ROUNDS];
    _rec.cpu_stress_threads = _ct.stress_threads_realised;
    _rec.cpu_stress_rounds = _ct.stress_rounds;
    _rec.cpu_stress_accesses = _ct.stress_accesses;
    _rec.cpu_preload_ops = _ct.preload_ops;
    _rec.cpu_noise_rounds = _ct.cpu_noise_rounds;
    _rec.cpu_noise_words = _ct.cpu_noise_words;
...
let dump_run_report ch =
  ...
  s {|    fprintf(stderr, "HetLitmus rendezvous: scored=%llu discarded=%llu (cap_cpu=%llu cap_gpu=%llu) caps=%lu/%u jitter=%d discard_max=%d%% %s\n",
            ...
            (unsigned long)_cap_cpu, _cap_gpu, (int)HET_RELEASE_JITTER_SPINS,
