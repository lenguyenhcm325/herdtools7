...
(* HetLitmus: the positive-control map
...
(* Read [csv] from [dir].  [None] = the file was not there, warned about at
   ...
   6-column header, or holds a row of another arity, is a Warn.user_error, a
   mis-bound map naming controls that are not controls.  [csv] is NOT
   hardwired: the lattice mu(T) weakens on is the CPU column's, so the lane's
   CPU frontend names the file (litmus/hetCpuFront.ml). *)
