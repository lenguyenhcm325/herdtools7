...
```
# full corpus: per-test PASS/FAIL table + tally (137 gpu-only, 338 het)
JOBS=8 bash hetlitmus/verify/l0_tokens.sh            # both
JOBS=8 bash hetlitmus/verify/l0_tokens.sh gpu-only   # 137
JOBS=8 bash hetlitmus/verify/l0_tokens.sh het        # 338
...
TALLY het:      338/338 PASS  (FAIL=0  GUARD-FAIL=0  ERROR=0)
```
...
fix: 338/338. No emitter mismatch was found — the lowering is faithful across the
whole corpus.
