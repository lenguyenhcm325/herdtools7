...
the cuda-litmus `MEM_STRESS` macro ships a bug; the cuda-litmus reuse is licence-cleared **with citation
required** (Anatole, 2026-07-06); the GPUHarbor + MC-Mutants secondhand claims were verified against the
...
kernel — **licence-cleared, MUST cite** cuda-litmus/Levine + the papers (S&D, Kirkham, Alglave'15,
MC-Mutants). **Fix the shipped `MEM_STRESS` macro bug** (it passes the wrong arg → mem-stress is a no-op in
...
2. The **het weak-behaviour hit-rate** — genuinely unknown (the 0.2 % was GPU-only).
...
- **Licence** — cuda-litmus reuse cleared for thesis use (Anatole, 2026-07-06) with **citation required**;
  Levine is the copyright holder, so a courtesy ack before the public artifact ships is prudent.
