...
corpus first — `tests/het/generate-x86.sh` resolves `$BIN/hetgen7` from
...
passing; the gate compares the fixture as a subset of a fresh `generate-x86.sh`
run and reports any extra, missing or differing file as DRIFT. If it reddens,
...
compared as a subset of `generate-x86.sh`'s output and never against
`generate.sh`.
