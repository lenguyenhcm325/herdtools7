...
#   Q2  gc coupling (symmetric meet: GPU producer)          DECLINE -> 19 rows
#   Q3  ARM DMB SY is a PTX fence.sc for Fence-SC order     DECLINE ->  2 rows
#   Q4  fence.{acquire,release}.sys semantics               DECLINE -> 11 rows
...
#
# Census 319/50/42 -> 319/18/74.  The 32 demoted rows are NO-ORACLE, never
...
#                                   acqrel      fence
#    MP, LB, S                      Disallowed  Disallowed  (cg cut only -- the
#                                               gc cut is NO-ORACLE under Q2)
...
#         build-nvidia-oracle.sh --declined-list  the 32-row contingency printout
...
# The demotion set, pinned.  32 = 19 gc (Q2) + 2 cg Fence-SC (Q3) + 11 cg
...
EXPECT_DEMOTED=32
EXPECT_DEMOTED_SHA="71ebe7baba4fbb83"      # sha256[:16] of the sorted 32 names
EXPECT_PRE_A=319 EXPECT_PRE_D=50 EXPECT_PRE_N=42     # pre-regeneration census
EXPECT_A=319     EXPECT_D=18     EXPECT_N=74         # shipping census
...
# ---------------------------------------------------------------------------
# THE NVOR DEMOTION CLASSES.  Five, because a demoted row must name EVERY slot
# its own derivation needs and 7 of the 19 gc rows need a second one.  The
# S_CUT_UNKEYED precedent (build-amd-oracle.sh): no blanket punting.
# ---------------------------------------------------------------------------
D_GC="cycle cut UNKEYED in the GPU-PRODUCER direction -- decision NVOR-Q2 (Nguyen 2026-08-06) DECLINED to register the SYMMETRIC MEET. The cg direction is registered (Q1) but the meet CMCM 4.6 leaves open is architecture-specific and CMCM footnote 7 p153:13 warns it CAN BE ASYMMETRICAL. Every scrap of GH200 system-scope het evidence is cg-direction: Bagchi Table 4 has GPU_rlx in the Consumer column of all 13 CPU-producer rows and GPU_acq nowhere at all and its only GPU_rel rows (21-22) are Y=gpu never Y=system; Fig 4a is CPU-producer; the sect 3.2 derivation is CPU-producer. These rows rest on framework text alone. NO-ORACLE not Allowed -- silence is not permission. THIS IS A PRE-REGISTERED FALSIFIABLE PREDICTION: observing the listed outcome on GH200 is DATA never a refutation of the compound model. NVOR_ACCEPT_DECLINED=1 restores the pre-regeneration Disallowed (contingency: build-nvidia-oracle.sh --declined-list) [decision NVOR-Q2 gc-meet DECLINED; CMCM 4.6 p153:13-14 -- admissible sect 3-4 framework naming the open slot; CMCM fn7 p153:13 the meet can be asymmetrical; Bagchi 4.2 Tab4 GAP -- no correctly sys-annotated GPU producer row exists; NVOR-register.md]"
...
D_GC_SC="cycle cut UNKEYED TWICE -- decisions NVOR-Q2 and NVOR-Q3 (Nguyen 2026-08-06). (i) Q2 DECLINED the symmetric meet so the GPU-producer direction has no registered coupling. (ii) Q3 DECLINED the cross-ISA identification: this cycle carries no rf at all so it is cut only through PTX's FENCE-SC ORDER which PTX ISA 9.3 8.9.3 defines over pairs of MORALLY STRONG fence.sc OPERATIONS -- strictly stronger than a sys-scoped fence and a claim that an ARM DMB SY IS a PTX fence.sc which no PTX text supports and which CMCM's operational LOST-POP sidesteps by having no Fence-SC object at all. A third independent reason stands on the R shape: the final-value condition presupposes a co edge between two RACY writes which PTX ISA 9.3 8.9.6 says are UNRELATED in coherence order. NO-ORACLE not Allowed. Observing the listed outcome on GH200 is DATA never a refutation. NVOR_ACCEPT_DECLINED=1 restores the pre-regeneration Disallowed [decision NVOR-Q2 gc-meet DECLINED; decision NVOR-Q3 sc-identification DECLINED; CMCM 4.6 p153:13-14 -- admissible sect 3-4 framework naming the open slot; CMCM fn7 p153:13 the meet can be asymmetrical; PTX ISA 9.3 8.9.3 Fence-SC order; PTX ISA 9.3 8.9.6 coherence-order; NVOR-register.md]"
D_SC="cycle cut UNKEYED -- decision NVOR-Q3 (Nguyen 2026-08-06) DECLINED to register that an ARM DMB SY IS a PTX fence.sc. This cycle carries no rf edge at all so the only route that cuts it is PTX's FENCE-SC ORDER which PTX ISA 9.3 8.9.3 defines as relating every pair of MORALLY STRONG fence.sc OPERATIONS -- strictly stronger than the sys-scoped-fence premise Q1 registers and a cross-ISA identification for which PTX offers no route (its sect 8.9.4 pairs a fence.sc only with another fence.sc). CMCM's operational LOST-POP has no Fence-SC object so the CMCM route sidesteps the question rather than answering it. On the R shape a second independent reason stands: the final-value condition presupposes a co edge between two RACY writes which PTX ISA 9.3 8.9.6 says are UNRELATED in coherence order. The GPU half of these rows is the BEST-supported in the corpus -- the weakness is entirely ARM-side. NO-ORACLE not Allowed. Observing the listed outcome on GH200 is DATA never a refutation. NVOR_ACCEPT_DECLINED=1 restores the pre-regeneration Disallowed [decision NVOR-Q3 sc-identification DECLINED; PTX ISA 9.3 8.9.3 Fence-SC order; PTX ISA 9.3 8.9.6 coherence-order; CMCM 4.6 p153:13-14 -- admissible sect 3-4 framework naming the open slot; NVOR-register.md]"
...
two_sided_order_pair() {
  ...
  # does the row need, and are they registered?
  local slots=""
  [ "$d0" = g ] && slots="$slots GC"
  ...
  if [ -n "$NVOR_SLOTS" ] && [ "$NVOR_ACCEPT_DECLINED" = 0 ]; then
    ...
    case "$NVOR_SLOTS" in
      ...
      "GC SC")      CLASS=S_GC_SC_UNKEYED;       SOURCE="$D_GC_SC";;
...
# EXACTLY the 32 demoted rows and NOTHING else: every toggled row goes
...
roundtrip() {
  ...
  for f in $(corpus); do
    ...
    n_tog=$((n_tog+1))
    case "$c0" in
      S_GC_MEET_UNKEYED|S_GC_UNIDIR_UNKEYED|S_GC_SC_UNKEYED|S_SC_IDENT_UNKEYED|S_UNIDIR_UNKEYED)
...
case "$MODE" in
  --declined-list)
    ...
    echo "running -- the 19 GC rows in particular are a PRE-REGISTERED, hardware-"
  ...
  --roundtrip)
    ...
    exit 0;;
  --generate|"") ;;
  *) die "unknown mode '$MODE' (expected --generate | --declined-list | --roundtrip)";;
esac
...
declare -A NVERD=() NCLASS=()
{
  ...
  echo "# 319/18/74.  32 rows that used to be Disallowed are NO-ORACLE because the"
  echo "# adjudication DECLINED to register the gc-direction meet (19 rows), the"
  echo "# ARM-DMB-SY-is-a-PTX-fence.sc identification (2) and the unidirectional-"
  echo "# fence semantics (11).  OBSERVING ONE OF THOSE OUTCOMES ON HARDWARE IS"
  echo "# DATA, NEVER A REFUTATION of the compound model; the 19 gc rows are a"
  echo "# PRE-REGISTERED falsifiable prediction.  Per-row slots:"
...
# strings it carries.  A count alone would survive demoting the wrong 32 rows.
if [ "$NVOR_ACCEPT_DECLINED" = 0 ]; then
  ndem=0
  for _s in "$D_GC" "$D_GC_UNIDIR" "$D_GC_SC" "$D_SC" "$D_UNIDIR"; do
  ...
  [ "$dsha" = "$EXPECT_DEMOTED_SHA" ] ||
    die "the demoted row SET has sha $dsha but the register pins $EXPECT_DEMOTED_SHA -- the gate moved a DIFFERENT 32 rows"
