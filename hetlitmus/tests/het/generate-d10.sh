...
# Mu is \`none' on every row and that is a DERIVED absence, not a gap: an x86
# rendering of these cycles carries plain MOVs and no fence, so it is already at
# the bottom of the x86 strength lattice and has no weakening left.  Every D10
# null is therefore a WEAK-NULL by construction, vouched for by the Layer-B
# canary ($CANARY) alone.
Test,Mu,MuRule,MuAlt,MuRelaxed,Canary
EOF

n=0
printf '%s\n' "$D10_ROWS" | while IFS=: read -r shape nprocs cycle; do
  [ -n "$shape" ] || continue
  ...
  echo "$name,none,at the lattice floor -- an x86 rendering of this cycle is plain MOVs with no fence to drop; Layer-B canary only,-,none,$can" >> "$CM"
done

n="$(ls "$OUT"/*.litmus | wc -l)"
echo "generate-d10: $n CPU-only tests + control-map-amd.csv in $OUT"
