...
#
# The oracle is expected-nvidia.csv (Model NVIDIA-PTX-AArch64), regenerated by
# build-nvidia-oracle.sh; the PLDI'23 expected.csv is AMD-GCN3+x86 and is not
# reused.  One-sided tests are Allowed/NO-ORACLE; (D) and (E) carry the
...
# one role only.  Grounding: docs/het-oracle.md.
...
# why only 2-proc shapes (minus 2+2W) and one cut for SB/LB.  Verdicts are
# computed compositionally by build-nvidia-oracle.sh and machine-checked by
# verify/ordercheck.py (make hetlitmus-order).
