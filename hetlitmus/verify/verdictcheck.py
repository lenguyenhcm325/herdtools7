...
nothing -- or worse, do the wrong thing loudly:
  ...
  * a rule that does not know WHAT THE MODEL PREDICTS frames every test as
    ...
    ... a single sighting REFUTES the model" on the 322 of 338 tests where the
    weak outcome is EXPECTED (286 Allowed) or where the model is SILENT (36
...
drift), feeds it synthetic het_obs_records, and asserts, in three phases:
  ...
  PHASE 3 -- THE EMITTED CORPUS
    All 338 het harnesses are emitted and their `_rec.het_oracle = ORACLE_*' is
    cross-checked, PER TEST, against field 2 of tests/het/control-map.csv, with a
    census (16 Disallowed / 286 Allowed / 36 NO-ORACLE / 0 UNSET).  A rule that
    branches correctly on an oracle class the emitter never sets is worthless: the
    unit test would pass and all 338 harnesses would still fail closed.
...
CENSUS = {"Disallowed": 16, "Allowed": 286, "NO-ORACLE": 36}
...
# the compound model -- on 322 of the 338 harnesses, including the canary itself.
#
...
CASES = [
    # =======================================================================
    # 1. THE DISALLOWED FRAME (16 tests).  Unchanged by B6c: the null IS the
    ...
    # =======================================================================
    # 2. B6c -- THE ALLOWED FRAME (286 tests).  Here the SIGHTING is the
    ...
    # model result.  This verdict is why the 286 needed a canary at all: without one
    ...
    # =======================================================================
    # 3. B6c -- THE NO-ORACLE FRAME (36 tests).  Q4 R5: characterization, NEVER
    ...
    # reachable here.  What the 36 can never produce is a MODEL claim -- Phase 2
    # enforces that.
    ...
    # have not fired.  Raised on the 322 tests that have no mutant by construction, it
...
def scan_prints(blocks, cases_by_name, quiet):
    ...
    # The 36 NO-ORACLE rows must never make a MODEL claim in either direction.
    for name, (oracle, text) in sorted(blocks.items()):
        if oracle != "NO-ORACLE":
            continue
...
def bite():
    ...
    try:
        ...
        # if it is ever RUN.  The census is what stops 1 of 338 quietly claiming
